...
<html lang="en">
    ...
    <body class="bg-gray-100">
        ...
        }" x-init="fetchCategoryData(); fetchProductData()">
                  ...
                  Add Category
                    ...
                    <div class="mt-8">
                        <ul class="flex flex-wrap">
                            ...
                            <a
                    x-on:click.prevent="toggleProductForm(category.id)"
...
<!-- 
    <div class="mt-8">
                <ul class="flex flex-wrap">
                    <template x-for="category in categories" :key="category.id" x-if="selectedCategory && category.parent_id == selectedCategory">
                        <a x-on:click.prevent="toggleProductForm(selectedCategory)"
                            class="ml-2 text-red-500 hover:text-red-700">
                            <li class="flex items-center mr-4 mb-4">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-purple-500 mr-2"
                                    viewBox="0 0 24 24" fill="currentColor">
                                    <path
                                        d="M10 2H4C2.89543 2 2 2.89543 2 4V20C2 21.1046 2.89543 22 4 22H20C21.1046 22 22 21.1046 22 20V8H14C12.8954 8 12 7.10457 12 6V2H10Z" />
                                </svg>
                                <span x-text="category.name" class="text-gray-800"></span>
                                <button x-on:click.prevent="deleteSubCategory(category.id)"
                                    class="ml-2 text-red-500 hover:text-red-700">
                    ...
                    <template x-for="product in products" :key="product.id">
                        <li class="flex items-center mr-4 mb-4">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-purple-500 mr-2"
                                viewBox="0 0 24 24" fill="currentColor">
                                <path
                                    d="M20.586 7H17V3.414L20.586 7ZM20 10V20C20 21.1046 19.1046 22 18 22H6C4.89543 22 4 21.1046 4 20V4C4 2.89543 4.89543 2 6 2H14L20 8V10Z" />
                            </svg>
                            <span x-text="product.name" class="text-gray-800"></span>
                            <button x-on:click.prevent="deleteProduct(product.id)"
                                class="ml-2 text-red-500 hover:text-red-700">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24"
 ...
 <!-- 
     x-init="fetchSubCategoryData(selectedCategory); fetchProductData()"


                 toggleProductForm(selectedCategory) {
            ...
            deleteProduct: function (id) {
                ...
                axios.get('/view?parent_id=${selectedCategory}')
                    .then((response) => {
                        this.categories = response.data;
                    })
                    .catch((error) => {
            ...
            fetchProductData() {
    axios.get(`/products/view?subcategory_id=${this.subcategory_id}`)
        .then((response) => {
            this.products = response.data.products;
        })
        .catch((error) => {
...
<html lang="en">
    ...
    <body class="bg-gray-100">
        <div class="flex justify-center items-center min-h-screen" x-data="{
            ...
            submitProductForm() {
                const parentCategory = this.categories.find(category => category.id === parseInt(this.formData.parent_id));
                this.formData.parent_id = parentCategory ? parentCategory.id : null;
                axios.post('/products', this.formData)
                    .then((response) => {
                        console.log('Form submitted:', response.data.message);
                        this.formData.name = '';
            ...
            fetchProductData() {
                axios.get(`/products/view?id=${this.selectedSubCategoryId}`)
                    .then((response) => {
                        this.products = response.data.products;
                    })
                    .catch((error) => {
        ...
        }"  x-init="fetchSubCategoryData(); fetchProductData()">
        <div class="w-full md:w-2/3 lg:w-1/2 bg-white shadow rounded-lg">
            <div class="p-8">
            <template x-if="isAddingProduct">
                    <form x-on:submit.prevent="submitProductForm()">
            ...
            <template x-if="!isAddingProduct">
                <form x-on:submit.prevent="submitSubCategoryForm()">
                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-bold mb-2">
                            Name:
                        </label>
            ...
            <div class="mt-8">
                <ul class="flex flex-wrap">
                    <template x-for="category in categories" :key="category.id">                
                    <a x-on:click.prevent="toggleProductForm(category.id)"
                    class="ml-2 text-red-500 hover:text-red-700">
                        <li class="flex items-center mr-4 mb-4" x-show="category.parent_id == localStorage.getItem('categoryId')">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-purple-500 mr-2"
                                    viewBox="0 0 24 24" fill="currentColor">
                                    <path
                                        d="M10 2H4C2.89543 2 2 2.89543 2 4V20C2 21.1046 2.89543 22 4 22H20C21.1046 22 22 21.1046 22 20V8H14C12.8954 8 12 7.10457 12 6V2H10Z" />
